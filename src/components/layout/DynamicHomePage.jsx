...
export default function DynamicHomePage({
  ...
  useEffect(() => {
    // Use the database products passed from the server
    const allProducts =
      staticAllProducts.length > 0 ? staticAllProducts : lsGetProducts([]);
    ...
    // 2. Wall Hangings
    setWallHangings(allProducts.filter((p) => p.category === "wall-hangings"));

    // 3. Table Top / Décors
    setTableTop(allProducts.filter((p) => p.category === "table-top-mount"));

    // 4. Wall & Table Combo
    setCombos(allProducts.filter((p) => p.category === "wall-table-combo"));

    // 5. Gift Items
    setGiftItems(allProducts.filter((p) => p.category === "gift-items"));
  ...
  }, [staticAllProducts, staticFeatured, staticTestimonials]);

  const promoBannerText =
    siteConfig?.promoBannerText ??
    "Special Offer: Get FLAT 40% OFF on your first order! Use code: FIRST40";
  const promoBannerActive = siteConfig?.promoBannerActive ?? true;

  return (
    <>
      {/* ── PROMOTION BANNER (admin-controlled) ── */}
      {promoBannerActive && (
        <div
          style={{
      ...
      <CategorySection
        ...
        description={
          staticCategories.find((c) => c.slug === "table-top-mount")?.description
        }
        products={tableTop}
      ...
      <CategorySection
        id="gifts"
        title={
          staticCategories.find((c) => c.slug === "gift-items")?.label ||
          "Gift Items"
        }
        overline="Thoughtful Treasures"
        description={
          staticCategories.find((c) => c.slug === "gift-items")?.description
        }
        products={giftItems}
            ...
            <div className="container">
              <div className="section-header">
                ...
                <p className="text-body">
                  Commission divine idols of your choice — any deity, any size, any finish.
                </p>
              </div>
              ...
              <div style={{
                ...
                <div style={{ position: "relative", minHeight: 360 }}>
                  ...
                  <div style={{
                    ...
                    background: "linear-gradient(135deg, rgba(139,0,0,0.25) 0%, transparent 60%)",
                  }} />
                  <span style={{
                ...
                {/* Content column */}
                <div style={{ padding: "var(--space-10) var(--space-8)", display: "flex", flexDirection: "column", gap: "var(--space-4)" }}>
                  <span style={{
                    ...
                    <svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
                      <path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14l-5-4.87 6.91-1.01L12 2z" />
                    </svg>
                  </span>
                  <h3 style={{ fontFamily: "var(--font-heading)", fontSize: "var(--fs-24)", fontWeight: 800, color: "var(--text-dark)", lineHeight: 1.2 }}>
                    {feature.title}
                  </h3>
                  <p className="text-body" style={{ lineHeight: "var(--lh-loose)", color: "var(--text-muted)" }}>
                    {feature.description}
                  </p>
