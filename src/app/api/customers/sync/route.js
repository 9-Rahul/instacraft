  ...
  try {
    ...
    if (firebaseUid === 'internal-admin-001') {
      return NextResponse.json({
        ...
        customer: { id: 0, name: "System Admin", email: "Admin@ishtacrafts.in" },
        isAdmin: true,
      }, { status: 200 });
    }
    ...
    const rows = await db.query("SELECT * FROM customers WHERE firebase_uid = ?", [firebaseUid]);
    let syncedCustomer;
    ...
    if (rows.length > 0) {
      ...
      const updatedRows = await db.query("SELECT * FROM customers WHERE firebase_uid = ?", [firebaseUid]);
      ...
      const insertedRows = await db.query("SELECT * FROM customers WHERE id = ?", [insertResult.insertId]);
